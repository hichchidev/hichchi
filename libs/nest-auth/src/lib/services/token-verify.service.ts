...
export class TokenVerifyService {
    ...
    async getPasswordResetTokenByUserId(userId: string | number): Promise<string | number | undefined> {
        return await this.cacheService.get<string | number>(PASSWORD_RESET_USER_KEY(userId));
    }

    async getUserIdByPasswordResetToken(token: string | number): Promise<string | number | undefined> {
    ...
    async getEmailVerifyTokenByUserId(userId: string | number): Promise<string | number | undefined> {
        return await this.cacheService.get<string | number>(EMAIL_VERIFY_USER_KEY(userId));
    }

    async getUserIdByEmailVerifyToken(token: string | number): Promise<string | number | undefined> {
